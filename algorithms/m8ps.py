...
def heuristic(start, end, barriers, maxIterations=100000, animate=False, socketInformation=None): #function for something else
        ...
        try:
            ...
            if socketInformation != None:
                ...
                if 'io' in socketInformation:
                    socketInformation['io'].emit('message', { 'meta':{'algorithm':'Magnetic algorithm 16', 'visitSize':len(traces[0])+len(traces[1])}, 'gridSize':socketInformation.get('gridSize'), 'id':socketInformation.get('id'), 'path':path, 'barriers':socketInformation.get('stringBarriers'), 'visited':convertTracesIntoSingleObj(traces) })
    ...
    if mergePoint != None:
        ...
        if socketInformation != None and 'io' in socketInformation:
            ...
            socketInformation['io'].emit('message', { 'meta':{'algorithm':'Magnetic algorithm 16', 'visitSize':len(traces[0])+len(traces[1])}, 'gridSize':socketInformation.get('gridSize'), 'id':socketInformation.get('id'), 'barriers':socketInformation.get('stringBarriers'), 'path':tempPath, 'visited':convertTracesIntoSingleObj(traces) })
